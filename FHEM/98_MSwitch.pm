...
my $vupdate      = 'V6.2';					# versionsnummer der datenstruktur . änderung der nummer löst MSwitch_VersionUpdate aus .
		...
		while (<HELP>) 
			{
				...
				if ($pfad eq "System") {$_ =~ s/\n//g; $data{MSwitch}{Widget}{$widgetname}{system} .= $_;  } 
...
Log3("MSwitch",5,"Messages collected while initializing MSwitch-System:\n$startmessage");
...
sub MSwitch_Shutdown($) {
	...
	if ($shutdowned ==  $anzahlmswitch && $foundcfgs > 0)
	{
		if ( AttrVal( 'global', 'autosave', '1' ) eq "1")
		{
		MSwitch_LOG( $Name, 1, "MSwitch -> write cfg");
		fhem("save");
		}
	}
...
sub MSwitch_summary($) {
    ...
    if ( @found_devices > 0)
	{
		 $testroom= ReadingsVal( $found_devices[0], 'MSwitch_inforoom', 'undef' );
	}
	else
	{
		 $configdevice ="undef";
...
sub MSwitch_DefineHEX($$$) {
	...
	readingsSingleUpdate( $hash, "waiting", ( time + 10 ),0 );
	MSwitch_defineWidgets($hash);    #Neustart aller genutzten widgets
	MSwitch_Createtimer($hash);    #Neustart aller timer	
	return;
}
...
sub MSwitch_Make_Undo($) {
	...
	my @found_devices = devspec2array("TYPE=MSwitch:FILTER=.msconfig=1");
     if ( @found_devices > 0)
	 {
		 return if (ReadingsVal( $found_devices[0], 'MSwitch_Undo', 'off' ) eq "off");
	 }
	 else
	 {
		 return; 
...
## anzeige funktionserkennung
        ...
        if ( $errorstring ne '' ) {
			...
			MSwitch_LOG("test",1,$errorstring);
            chop $err2;
            chop $err2;
            $err2 = substr( $err2, 1 );
    ...
    if ( $opt eq 'HTTPresponse' ) {
        ...
        my $window = "<textarea cols='120' rows='30'>".$data{MSwitch}{$name}{HTTPresponse}."</textarea>";

        if ( length($inhalt) <1 )    # wenn ein Fehler bei der HTTP Abfrage aufgetreten ist
        {
            return  "<html>Keine Daten vorhanden</html>";
        }
        return "<html><span style=\"font-size: medium\">" . $window . "<\/span></html>";
    }
  ...
  #delays
        ...
        foreach my $a ( sort keys %{$timehash} ) 
		{
           # my $b      = substr( $hash->{helper}{delays}{$a}, 0, 10 );
            ...
            $ret .= "<strong>Indikator: </strong>" . $hash->{helper}{delaydetails}{$a}{Indikator} . "<br>";
			 $ret .= "<strong>Name: </strong>" . $hash->{helper}{delaydetails}{$a}{name} . "<br>";
            $ret .= "<strong>auszuführender Befehl:</strong><br>". $hash->{helper}{delaydetails}{$a}{cmd} . "<br>";
            $ret .= "</div><hr>";
			}
...
### modulmode - no sets
	...
	{
        return "Unknown argument $opt, choose one of config:noArg support_info:noArg active_timer:show $statistic";
    }
#######
    ...
    if ( AttrVal( $name, 'MSwitch_Mode', 'Notify' ) eq "Dummy" ) 
	{
		...
		{
            return "Unknown argument $opt, choose one of support_info:noArg restore_MSwitch_Data:noArg $extension $statistic";
        }
    }

    if ( ReadingsVal( $name, '.lock', 'undef' ) ne "undef" ) {
        return"Unknown argument $opt, choose one of HTTPresponse:noArg support_info:noArg active_timer:show,delete config:noArg restore_MSwitch_Data:noArg $statistic ";
    }
    else 
	{
        return "Unknown argument $opt, choose one of HTTPresponse:noArg Eventlog:sequenzformated,timeline,clear support_info:noArg config:noArg active_timer:show,delete restore_MSwitch_Data:noArg $extension $statistic";
    }
}
...
sub MSwitch_Get_Statistik($) {
	...
	$re.="<td>&nbsp;&nbsp;&nbsp;<\/td>";	
	$re.="<td> ".$hash->{helper}{statistics}{notifyloop_blocked_from_grep}."<\/td>";
	$re.="<\/tr>";
	
	$re.="<tr>";
	$re.="<td>{Notify_FN} von GREP gefilterte Einzelvents aus Paketen :<\/td>";
	$re.="<td>&nbsp;&nbsp;&nbsp;<\/td>";	
...
sub MSwitch_Set_wizard1($@)  
	{
		my ( $hash, $name, $cmd, @args ) = @_;
		FW_directNotify( "FILTER=$name","#FHEMWEB:WEB", "setargument('$args[0]', '$args[1]')", "");
		return;	
	}
...
sub MSwitch_Set_ChangeRenamed($@)  
    ...
    if ( @found_devices > 0)
	{
	FW_directNotify( "FILTER=$found_devices[0]","#FHEMWEB:WEB", "information('Device \"$name\" angepasst ($args[0]->$args[1])...')", "");
	}
	return;
	}	
    ...
    if ( $args[0] ne 'ID' || $args[0] ne '' ) 
		{
            if ( $args[1] !~ m/\d/ ) 
			{
                MSwitch_LOG( $name, 1,"error at id call $args[1]: format must be exec_cmd_1 <ID x,z,y>");
                return;
            }
        }
        # cmd1 abarbeiten
...
sub MSwitch_Set_Statecounter($@)
{
	...
	if ( AttrVal( $name, "MSwitch_State_Counter", "off" ) eq "off" || AttrVal( $name, "MSwitch_Mode", "Notify" ) eq "Notify" )
	{
	return;	
	}
	my $showevents = AttrVal( $name, "MSwitch_generate_Events", 0 );
...
readingsSingleUpdate( $hash, "off_count", $offcount, $showevents );
	...
	if (($cmd eq "off" && $oldstate eq "on") || ($cmd eq "on" && $oldstate eq "on"))
	{
...
if ( $cmd eq 'fullrestore') 			{my $ret = MSwitch_fullrestore( $hash, $args[0] );return $ret;}
if ( $cmd eq 'fullrestorelocal') 		{my $ret = MSwitch_fullrestorelocal( $hash, $args[0] );return $ret;}
if ( $cmd eq 'getbackup') 				{my $ret = MSwitch_Get_Backup( $hash );return $ret;}
if ( $cmd eq 'fullbackup') 				{my $ret = MSwitch_FullBackup( $hash, $args[0]);return $ret;}
if ( $cmd eq 'getbackupfile') 			{my $ret = MSwitch_Get_Backup_inhalt( $hash,$args[0] );return $ret;}
if ( $cmd eq 'getdevices') 				{my $ret = MSwitch_Get_Devices( $hash );return $ret;}
...
if ( $cmd eq 'del_history_data' ) 		{MSwitch_Set_DelHistory($hash, $name, $cmd, @args);return;}
if ( $cmd eq 'backup_MSwitch' ) 		{MSwitch_backup_this($hash,"");return;}
...
if ( $init_done && $aName eq 'MSwitch_State_Counter' ) {
	...
	MSwitch_LOG( $name, 6, "Aufruf createtimer" );
	MSwitch_LOG( $name, 6,"DELETE Statereadings");
...
# startverzögerung abwarten
	...
	{
		$own_hash->{helper}{statistics}{notifyloop_startdelay_blocked}++ if $statistic ==1; #statistik
...
# test wait attribut
    if ( ReadingsVal( $ownName, "waiting", '0' ) > time ) 
	{
        ...
        # teste auf attr waiting verlesse wenn gesetzt
		$own_hash->{helper}{statistics}{notifyloop_wait_blocked}++ if $statistic ==1; #statistik
        return "";
    }
    else 
	{
...
# aktuelle zeit in sekunden errechnen 
		...
		# localtime:     Wed Sep 22 16:04:37 2021 
		$aktsectime =~ s/\s+/ /g;
...
# nur wenn attribut gesetzt
	...
	#################	
		...
		{
		MSwitch_LOG($ownName,6,"Key $line");
		}
		##################################################
		
		my $checklast = $eventteile[0].":".$eventteile[1].":.*";
		MSwitch_LOG($ownName,6,"checklast $checklast");
		...
		my $newdiff =  $lastincomming+$eventsollwaitwild-time;
			if ($newdiff > 0)
			{
				...
				MSwitch_LOG($ownName,6,"Event $checklast wird noch $newdiff sekunden blockiert");
				next EVENT;
			}
			else
			{
...
##########################
	...
	#	check last eventpart mit bedingung
		...
		MSwitch_LOG($ownName,6,"LESEVERSUCH: {MSwitch}{$ownName}{eventwaitwild}{$checklast}");
		#{$checklast} 
		foreach my $line (keys %{ $data{MSwitch}{$ownName}{eventwaitwild}{$checklast}})
		{
		MSwitch_LOG($ownName,6,"Key $line -> ".$data{MSwitch}{$ownName}{eventwaitwild}{$checklast}{$line});
		my $eventsollwaitwildcond =$data{MSwitch}{$ownName}{eventwaitwild}{$checklast}{$line};	
		my $condition = $line;
		$condition =~ s/\*/$eventteile[2]/g;
		MSwitch_LOG($ownName,6,"condition $condition ");
        my $result = eval ($condition);
		
		MSwitch_LOG($ownName,6,"result $result ");
		if ($result)
		{
			MSwitch_LOG($ownName,6,"CONDITION ok ");
			### bedingung erfüllt
			...
			if ($newdiff > 0)
			{
				my $lasttime= time+$eventsollwaitwildcond - $lastincomming ;
				MSwitch_LOG($ownName,6,"Event $eventcopy wird noch $newdiff sekunden blockiert");
				next EVENT;
			}
			else{
				$data{MSwitch}{$ownName}{inputeventwaitwild}{$ersatzevent} = time;
			}
...
#############################################################      
			...
			if (    $chbridge eq "bridge found" )
					{
						MSwitch_history($own_hash,$ownName,$showevents,$devName,$aktsectime,$evhistory,$eventcopy);
						next EVENT
					}
            }
############################################################################################################
...
my $timestampreading =  AttrVal( $ownName, 'MSwitch_Eventhistory_timestamp_to_Reading', "0" );
...
#################################################	
     ...
     #schaltet zweig 3 und 4
        ...
        if ( $events ne "" )
		{
			...
			MSwitch_LOG( $ownName, 6,"exec setdata $events");
        }
...
sub MSwitch_history(@) {
	 ...
	 my ( $own_hash,$ownName,$showevents,$devName,$aktsectime,$evhistory,$eventcopy ) = @_;
				...
				if ( ReadingsVal( $ownName, ".Trigger_device", "no_trigger" ) eq 'all_events' )
				{
				@eventfunction = split( / /, $own_hash->{helper}{eventhistory}{$evde}{$evreading} ) if exists $own_hash->{helper}{eventhistory}{$evde}{$evreading};
				@eventfunctiontime = split( / /, $own_hash->{helper}{eventhistory}{timer}{$evde}{$evreading} ) if exists $own_hash->{helper}{eventhistory}{timer}{$evde}{$evreading};

				}
				else
				{
				@eventfunction = split( / /, $own_hash->{helper}{eventhistory}{$evreading} ) if exists $own_hash->{helper}{eventhistory}{$evreading};
				@eventfunctiontime = split( / /, $own_hash->{helper}{eventhistory}{timer}{$evreading} ) if exists $own_hash->{helper}{eventhistory}{timer}{$evreading};
			...
			if ( ReadingsVal( $ownName, ".Trigger_device", "no_trigger" ) eq 'all_events' )
			{
            $own_hash->{helper}{eventhistory}{$evde}{$evreading}=$neweventfunction;	
			$own_hash->{helper}{eventhistory}{timer}{$evde}{$evreading}=$neweventfunctiontime;	
			}
			else
			{
				$own_hash->{helper}{eventhistory}{$evreading} =$neweventfunction;	
				$own_hash->{helper}{eventhistory}{timer}{$evreading} =$neweventfunctiontime;
			}
				
		my $timestampreading =  AttrVal( $ownName, 'MSwitch_Eventhistory_timestamp_to_Reading', "0" );
		my $realtimerreading =  AttrVal( $ownName, 'MSwitch_Eventhistory_realtime_to_Reading', "0" );
				...
				readingsBeginUpdate($own_hash);	
					foreach my $testdevices (@eventfunction) 
					{
						...
						my $realtime;
						$realtime =  FmtDateTime($eventfunctiontime[$count]) if $realtimerreading  == 1;;
						if ( ReadingsVal( $ownName, ".Trigger_device", "no_trigger" ) eq 'all_events' )
						{
						readingsBulkUpdate( $own_hash, $devName."_".$readname, $testdevices );
						readingsBulkUpdate( $own_hash, $devName."_".$readnametime, $eventfunctiontime[$count] ) if $timestampreading == 1;
						readingsBulkUpdate( $own_hash, $devName."_".$readnamerealtime, $realtime ) if $realtimerreading  == 1;

						}
						else
						{
						readingsBulkUpdate( $own_hash, $readname, $testdevices );
						readingsBulkUpdate( $own_hash, $readnametime, $eventfunctiontime[$count] ) if $timestampreading == 1;
						readingsBulkUpdate( $own_hash, $readnamerealtime, $realtime ) if $realtimerreading  == 1;
...
sub MSwitch_checkbridge($$$) {
	...
	#	
		...
		if ($foundcond eq "1")
		{
			...
			MSwitch_LOG( $name, 6,"Aufruf checkcondition : ".__LINE__);
...
sub MSwitch_fhemwebconf($$$$) {
    ...
    my $devstring;
    my $cmds;
    $cmds .="' reset_Switching_once loadHTTP timer:on,off del_repeats reset_device active del_function_data del_history_data:noArg inactive on off del_delays backup_MSwitch fakeevent exec_cmd_1 exec_cmd_2 wait del_repeats reload_timer change_renamed reset_status_counter:noArg reset_cmd_count ',";
    ...
    for (@found_devices)
	{
		...
		if ( $test =~m/.*'.*/ )
		{
			MSwitch_LOG($Name,1,"der Fhembefehl 'getAllSets' verursacht eine ungültige Rückgabe des Devices $_ , bitte den Modulautor informieren . Hierfür bitte den Devicetypen des Devices $_ angeben. ");
			$test=~ s/'//g;
		}	
        $cmds      .= "'" . $test . "',";
        $devstring .= "'" . $_ . "',";
...
sub MSwitch_fhemwebFn($$$$) {
	...
	if (AttrVal( $Name, 'MSwitch_Comment_to_Room', '0' ) eq "1" || $showcom eq "on" )
	  {
		 my $current = AttrVal( $Name, "comment", "" ); 
		 if ($current ne ""){
		$comsystem= "<table border='0'  cellpadding='4' style='border-spacing:0px;'>
				<tr>
				<td>&nbsp;</td>
	...
	if ( AttrVal( $Name, 'MSwitch_SysExtension', "0" ) =~ m/(0|1)/s && exists $data{MSwitch}{$Name}{Ansicht} && $data{MSwitch}{$Name}{Ansicht} eq "room")
	{
    ...
    if ( AttrVal( $Name, 'MSwitch_SysExtension', "0" ) =~ m/(1|2)/s && ReadingsVal( $Name, '.sysconf', '' ) ne "" )
	{
		...
		if ( $system =~ m/\[Reading:(.*?)\]/s ) 
		{
			...
			{				
				...
				my $widget ="<span class=\"dval\" informid=\"$Name-$reading\">$current</span>";
		...
		if ( $system =~ m/\[Widget:(.*?)\]/s ) 
		{
			...
			{				
				...
				my $widget ="<div class='fhemWidget' cmd='".$reading."' reading='".$reading."' dev='".$Name."' arg='".$argumente."' current='".$current."'></div>";
				...
				while ( $system =~ m/\[MSwitch_Widget:$a(.*?)\]/s ) 
					{
						...
						{	
							...
							if (defined $args[1])
							{
								...
								my $current = ReadingsVal( $Name, $string[0], 'undef' );
								...
								$html =~ s/ARGS/$stringcompl[1]/g if defined $stringcompl[1];
							}
						}
						$system =~ s/\[MSwitch_Widget:$a$lastpart\]/$data{MSwitch}{Widget}{$a}{script}$html/g;
					}
            }
		}	
...
####################
	...
	if (!defined $devicetotriggerselect){
		 ...
		 MSwitch_LOG( $Name, 5, "$Name:undefined devicetotriggerselect - setze no_trigger" );
		$devicetotriggerselect="no_trigger";
	}
...
if  ($triggerdevice ne "no_device" && $triggerdevice ne "" && $triggerdevice ne "MSwitch_Self" && $triggerdevice ne "all_events" && $triggerdevice ne "no_trigger")
{
...
if ($found1 == 0 && $found2 == 0)
	{
		...
		$alert1.="&nbsp;<input name='' type='button' value='umbenennen' onclick=\"rename('".$triggerdevice."')\">";
		$alert1.="</span>";
	}
}
...
for my $testname (@affectedklartext) {
  ...
  LOOP9: for my $name ( sort @found_devices ) {
	...
	# next LOOP9 if @gefischt <1;
		...
		#MSwitch_LOG($name, 4, "Device_Name ".$name." - Device_TYPE ".$devicehash->{TYPE} );
		my $deviceTYPE = "";
...
<tr name='MS-AKTDEVICE-EDIT' style='display:none;'>	<td colspan='3'>
	...
	<tr>
		<td></td>
		<td style='height: MS-cellhighdebug;width: 100%'>MS-TEST-1MS-CONDCHECK2</td>
	</tr>
	<tr>
		<td>MS-HELPexeccmd</td>
...
##########################
        ...
        foreach (@affecteddevices) {
		   ...
		   $alert.="&nbsp;<input name='' type='button' value='umbenennen' onclick=\"rename('".$aktdevice."')\">";
		   $alert.="</span>";
		   $cmdsatz{$devicenamet} = $savedetails{ $aktdevice . '_on' } . " " . $savedetails{ $aktdevice . '_off' };
            }
...
"<input type='button' id='' value='showdevices' onclick=\"javascript:showgroup('"
            ...
            if ( $expertmode eq '1' ) 
			{
                $NAMEsatz ="$zusatz $devicenamet $alert $realname&nbsp;&nbsp;$groupbutton&nbsp;&nbsp;$dalias";
...
$SET1PLAIN.="</table>";
                ...
                $SET1  = "<table width='100%' border ='0'><tr>
                  ...
                  . $nopoint
                  . "' onchange=\"javascript:
				  activate(document.getElementById('". $_. "_on').value, '". $_. "_on_sel', '". $cmdsatz{$devicenamet}. "', 'cmdonopt". $_. "1')
				  \" >
					<option value='no_action'>no_action</option>" . $option1html . "</select>
					</td>";
					
				$SET1 .= "<td><input type='$hidden' id='cmdseton"
                  . $_
                  . "' name='cmdseton"
                  . $nopoint
                  . "' size='30'  value ='"
				...
				if (AttrVal( $Name, 'MSwitch_use_WebWidgets', $webwidget )==1)
				{
				$SET1 .= "<td  align='right' width='100%'><input id='" . $_ . "_on_sel_but' type='button' value='Widget/Text' onclick='javascript: changeinput(\"".$_."_on_sel\",\"cmdonopt". $_. "1\",\"".$cmdsatz{$devicenamet}."\",\"".$_."_on\")''>&nbsp;</td>";
				}
				else{
...
## block off #$devicename
            ...
            if ( $devicenamet ne 'FreeCmd' )
			{
				...
				if (AttrVal( $Name, 'MSwitch_use_WebWidgets', $webwidget )==1)
				{
					$SET2 .= "<td  align='right' width='100%'><input id='" . $_ . "_off_sel_but' type='button' value='Widget/Text' onclick='javascript: changeinput(\"".$_."_off_sel\",\"cmdoffopt". $_. "1\",\"".$cmdsatz{$devicenamet}."\",\"".$_."_off\")''>&nbsp;</td>";
				}	  
					  else
					  {
...
$MStrigger = "<br><select style=\"width:700px;\" size='1' id =\"trigdevnew\" name=\"trigdevnew\">"
      . "</select>";
...
"<input type='text' id ='triggerwhite' name='triggerwhitelist' size='35' value ='"
    ...
    $MSonand1 =
      ...
      . $timeon . "' onClick=\"javascript:bigwindow(this.id,'web',3);\">";
    $MSonand2 =
      "<input type='text' id='timeoff' name='timeoff' size='35'  value ='"
      . $timeoff . "' onClick=\"javascript:bigwindow(this.id,'web',3);\">";
    $MSexec1 =
      "<input type='text' id='timeononly' name='timeononly' size='35'  value ='"
      . $timeononly . "' onClick=\"javascript:bigwindow(this.id,'web',3);\">";

    if ( $hash->{INIT} ne 'define' ) {
        $MSexec2 =
"<input type='text' id='timeoffonly' name='timeoffonly' size='35'  value ='"
          . $timeoffonly . "'onClick=\"javascript:bigwindow(this.id,'web',3);\">";

        $MSexec12 =
"<input type='text' id='timeonoffonly' name='timeonoffonly' size='35'  value ='"
          . $timeonoffonly . "' onClick=\"javascript:bigwindow(this.id,'web',3);\">";
    }
...
##############################################################
    ...
    $ret .="<div id='MSwitchWebTRDT' nm='$hash->{NAME}' cellpadding='0' style='border-spacing:0px;'>"
      . $triggerdetailhtml
      . "</div>";
...
####################
	...
	var webwidget= '" . AttrVal( $Name, 'MSwitch_use_WebWidgets', $webwidget ) . "';
...
## reset und timeline muss noch in javaweb uebernommen werden aber nicht wichtig !!
    ...
    if ( AttrVal( $Name, 'MSwitch_Modul_Mode', "0" ) eq '1' ) 
	{
        ...
        my $testname = "EVENT";
		#display:none
		$hidecode = "<div class='makeTable Radingcontainer' style='display:none'></div>";
	...
	if ( AttrVal( $Name, 'MSwitch_SysExtension', "0" ) =~ m/(2)/s  && $data{MSwitch}{$Name}{Ansicht} eq "room")
	{
		...
		for (@readlist) 
		{
            ...
            $inhalt .=
              ...
              . $Name . "-" . $_ . "\">" . $setinhalt . "</div></td>"
			  #."<td><div informid=\"". $Name . "-" . $_ . "-ts\">" . $settimstamp . "</div></td>"
			  ."</tr>";
            if ( $oddeven eq "odd" )
			{
                $oddeven = "even";
		...
		my $table="";
		if ($system ne ""){
		$table ="<br><div class='makeTable Radingcontainer' style='display:none'><table>$inhalt</table></div>";
		}
...
my ( $hash,$cs) = @_;
	...
	{	
		...
		MSwitch_LOG( $name, 6, "Snippetersetzung: $cs ");
	}
	return $cs;
}
...
sub MSwitch_Exec_Notif($$$$$) {
	...
	MSwitch_LOG( $name, 6,"it event : $event");
	
	MSwitch_LOG( $name, 6,"it comand : $comand");
...
my $field="";
	...
	if (!defined $devicedetails{$timerkey}){$devicedetails{$timerkey} =0;}
	...
	MSwitch_LOG( $name, 6,"it devicedetails{timerkey} : $devicedetails{$timerkey}");
	...
	$devicedetails{$timerkey} = timetoseconds($name,$devicedetails{$timerkey});
	if (!defined $devicedetails{$timerkey}){$devicedetails{$timerkey} =0;}
    if ( $devicedetails{$timerkey} =~ m/{.*}/ )
		...
		{
			MSwitch_LOG( $name, 6, "$name -  ZAHL gefunden $devicedetails{$timerkey} L:" . __LINE__ );

		}else 
		{
			MSwitch_LOG( $name, 6, "$name - KEINE ZAHL gefunden $devicedetails{$timerkey} L:" . __LINE__ );
...
#MSwitch_LOG( $name, 7,  "setze Verzögerung $timecond" );


#MSwitch_LOG( $name, 7,  "last cmd: ".$data{MSwitch}{$name}{setdata}{last_cmd} );

			if (exists $data{MSwitch}{$name}{setdata}{last_cmd} && $data{MSwitch}{$name}{setdata}{last_cmd} eq "cmd_1"){$field="_countdownon";}
			if (exists $data{MSwitch}{$name}{setdata}{last_cmd} && $data{MSwitch}{$name}{setdata}{last_cmd} eq "cmd_2"){$field="_countdownoff";}
 	
	#MSwitch_LOG( $name, 7,  "field: ".$field );
	 
	
	
	
			if (defined $devicedetails{ $device.$field } && $devicedetails{ $device.$field } ne "")
				{
					...
					my $counternumber = 0;
					$savename = $devicedetails{ $device.$field }."_".$counternumber;	
					
					## prüde timername auf bereits laufend
					while ( exists  $hash->{helper}{countdown}{$savename} && $hash->{helper}{countdown}{$savename}!=0)
					{
						$counternumber++;
						$savename = $devicedetails{ $device.$field }."_".$counternumber;
					...
					readingsBeginUpdate($hash);
					readingsBulkUpdate( $hash, "lastsetting_delay_ident", $savename);
					readingsBulkUpdate( $hash, "lastsetting_delay_cmd", $data{MSwitch}{$name}{setdata}{last_cmd});
					readingsBulkUpdate( $hash, "lastsetting_delay_time", $format);

					if ($jump > 0)
					{
					$hash->{helper}{countdown}{$savename}=$sek;
					readingsBulkUpdate( $hash, $savename, $format);
					
					if ( !exists $hash->{helper}{countdownstatus} ){$hash->{helper}{countdownstatus} = "inaktiv";}
					
					
#MSwitch_LOG( $name, 7, " --------- aktivstatus von $savename : ".$hash->{helper}{countdownstatus});
				
					
					if ( $hash->{helper}{countdownstatus} eq "aktiv" )
						{
							# hole zeit der nächsten ausführung
							my $nexttime = $hash->{helper}{countdownnexttime};
							my $akttime=gettimeofday();
							my $nextjump = int ($nexttime-$akttime);
							my $diff = $jump-$nextjump;
							
							my $ctime=$sek+$diff;
							$hash->{helper}{countdown}{$savename}=$ctime;
							
# MSwitch_LOG( $name, 7, " --------- Gesetzt aus aktiv: ".$savename."_". $ctime);
					...
					if ($hash->{helper}{countdownstatus} ne "aktiv")
						{
						#aktiviere countdown;
...
# MSwitch_LOG( $name, 7, " --------- Gesetzt aus nicht aktiv: ".$savename."_". $timecond);
...
# savename festlegen !
					my $timerset = "[TIMER][NAME_$savename][NUMBER_$timercounter]$msg2";
                    $timers[$timercounter] = $timecond;
                    push( @execute, $timerset );
                    $timercounter++;
    ...
    if ( defined $fullstring && exists $hash->{helper}{lastexecute} && AttrVal( $name, 'MSwitch_Switching_once', 0 ) == 1
    ...
    else  
	{
        ...
        foreach my $device (@execute)
		{
			...
			{
				MSwitch_LOG( $name, 6,"-$debugmode- Ausgefuehrte Befehl: -$device-");	
			}
            ...
            if ( $device =~ m/\[TIMER\].*/ )
			{
				$device=~ m/\[NAME_(.*?)\]/;
				my $identifier =$1;	
				MSwitch_LOG( $name, 6, "found Identifier: $identifier ");
				$device =~ m/\[NUMBER_(.*?)](.*)/;
                my $number = $1;
                my $string = $2;
				$string =~ s/#\[MSNL\]/\n/g;
				...
				####################		
				InternalTimer( $timecondition, "MSwitch_Restartcmdnew", $timecondition."-".$delaydetails[1] );
                next;
            }
...
## ersetze multicondition Format [NAME:READING_h0::10]
	...
	while ( $condition =~ m/(.*[START|AND|OR|\&\&|\|\|\s|\(|\)]?)(\[.*::\d{1,}\])(.*?[\d|"|\]])(\s[END|AND|OR|\&\&|\|\|].*)/ )    #z.b $WE
    {	
	my $part1 =$1;
	my $part2 =$2;
	my $part3 =$3;
	...
	{
	$middlepart = $middlepart . " [".$treffer[0].":".$treffer[1].$treffer[2]."] ".$treffer[4]." && ";	
	$treffer[2]++;
	#last if $treffer[2] > 100;
	}
	chop $middlepart;
	chop $middlepart;
	chop $middlepart;
	...
	$condition = $newcondition;
		...
		while ( $condition =~ m/(.*[START|AND|OR|\&\&|\|\|\s|\(|\)]?)(\[.*::\d{1,}_time\])(.*?[\d|"|\]])(\s[END|AND|OR|\&\&|\|\|].*)/ )    #z.b $WE
    {	
	my $part1 =$1;
	my $part2 =$2;
	my $part3 =$3;
	...
	###########
	my $middlepart ="";
	my @treffer = $multicondition =~ m/\[(.*):(.*)(\d+)::(\d+)(_time)\](.*)/gs;

	while ($treffer[2] <= $treffer[3])
	{
	$middlepart = $middlepart . " [".$treffer[0].":".$treffer[1].$treffer[2].$treffer[4]."] ".$treffer[5]." && ";	
	$treffer[2]++;
	#last if $treffer[2] > 100;
	}
	chop $middlepart;
	chop $middlepart;
	chop $middlepart;
	MSwitch_LOG( $name, 6,"middlepart - $middlepart");
	###########
	my $newcondition = $part1."(".$middlepart.") ".$part4;
	$condition = $newcondition;
	}
...
#############
	...
	while ( $change =~ m/(^\d\d:\d\d:\d\d\s|\s\d\d:\d\d:\d\d\s|\s\d\d:\d\d:\d\d$|^\d\d:\d\d:\d\d$)/ ) 
	{
		...
		$timecondtest =~ s/\s+/ /g;	
		my ( $tday, $tmonth, $tdate, $tn, $time1 ) = split( / /, $timecondtest );
		MSwitch_LOG( $name, 6,"'00', $MM, $HH, tdate $tdate, tmonth $tmonth, time1 $time1");
...
sub MSwitch_Checkcond_time($$) {
	...
	MSwitch_LOG( $name, 6,"time1 $timecond1:" . __LINE__ );
	...
	{
		MSwitch_LOG( $name, 6,"found opt 1 L:" . __LINE__ );
	...
	{
		MSwitch_LOG( $name, 6,"found opt 2 L:" . __LINE__ );
        use constant SECONDS_PER_DAY => 60 * 60 * 24;
...
sub MSwitch_Checkcond_history($$) {
    ...
    if ( $hash->{helper}{history}{eventberechnung} ne "berechnet" )    # teste auf vorhandene berechnung
    {
        foreach $seq ( sort { $b <=> $a } keys %{$log} )
		{
			...
			$hash->{helper}{history}{event}{$x}{EVENT} =$hash->{helper}{eventlog}{$seq};
...
sub MSwitch_Createtimer($) 
{
	...
	my ( $secx, $minx, $hourx, $mday, $monthx, $yearx, $wday, $ydayx, $isdstx ) =localtime( gettimeofday() );
...
###### setze timer TIMER
	...
	readingsSingleUpdate( $hash, "Next_Timer", "no_timer_today", 1 ) if scalar(@nexttimer) == 0;
	readingsSingleUpdate( $hash, "Next_Timer", $nt[1], 1 ) if scalar(@nexttimer) > 0;
	}
	else{	
...
my ( $Name,$hash,$date, $aktmonth, $aktyear ) = @_;
	...
	if ( AttrVal( $Name, "MSwitch_State_Counter", "off" ) eq "24_Hours" && AttrVal( $Name, "MSwitch_Mode", "Notify" ) ne "Notify" )
	{
	my $newask = timelocal( '59', '59', '23', $date, $aktmonth, $aktyear );
	$newask = $newask + 4;
    my $newassktest = FmtDateTime($newask);
		...
		foreach my $countdown ( keys %{$hash->{helper}{countdown}} )
			  {
				...
				$format =~ s/SS/00/g;	
				$format =~ s/ss/0/g;	
				readingsSingleUpdate( $hash, $countdown,$format , 1 ) ;
			...
			foreach my $a ( sort keys %{$delaydindikatorhash} )
			{
				...
				if ($checkname eq $indikator)
				{
					...
					if (   $expertmode eq '1')
						...
						$format =~ s/SS/00/g;	
						$format =~ s/ss/0/g;
						readingsSingleUpdate( $hash, $checkcounter,$format , 1 ) ;	
						}
				}
			}	
		}
			...
			foreach my $a ( sort keys %{$delaydindikatorhash} )
			{
				...
				if ($checkname eq $indikator)
				{
					...
					if (   $expertmode eq '1')
						...
						$format =~ s/SS/00/g;	
						$format =~ s/ss/0/g;
						readingsSingleUpdate( $hash, $indikator,$format , 1 ) ;
						}
				}
			}
		}
...
sub MSwitch_Check_Event($$) {
	...
	MSwitch_LOG( $Name, 6, "eingehend : $hash, $eventin  " );
    ...
    if ( $eventin ne $hash ) 
	{
		MSwitch_LOG( $Name, 6, "eventin ne hash  " );
        if ( ReadingsVal( $Name, '.Trigger_device', '' ) eq "all_events" ) 
		{
			MSwitch_LOG( $Name, 6, "TRIGGERDEVICE all_events  " );
            my @eventin = split( /:/, $eventin,3 );
			if (!defined $eventin[0]){$eventin[0] ="";}
			if (!defined $eventin[1]){$eventin[1] ="";}
			if (!defined $eventin[2]){$eventin[2] ="";}
			MSwitch_LOG( $Name, 6, "TRIGGERDEVICE eventin $eventin[0] " );
        ...
        else 
		{
			...
			MSwitch_LOG( $Name, 6, "TRIGGERDEVICE NE all_events  " );
            my @eventin = split( /:/, $eventin,3 );
    ...
    if ( $eventin eq $hash )
	{
		MSwitch_LOG( $Name, 6, "eventin eq hash  " );
...
# trigger enthält bedingung
		if  ($triggerfield =~ m/(.*)\[(.*)(\[.*?\])/ )
		{
			MSwitch_LOG( $ownName, 6,"SetMagic gefunden: $triggerfield  L:" . __LINE__ );
			my $begin =$1;
			my $start =$2;
			my $setmagic=$3;
			my $workmagic =  $3;
...
sub MSwitch_restore_this($$) {
	...
	if ($arg eq "backupfile"){
		open( BACKUPDATEI, "<".$restoredir."MSwitch_Device_".$Name.".txt" ) || return "no Backupfile found!\n";;
		while (<BACKUPDATEI>) 
		{
			$Zeilen = $Zeilen . $_;
		}
...
"<html><center> Bei Supportanfragen bitte untenstehene Datei anhängen, das erleichtert Anfragen erheblich.<br>&nbsp;<br><textarea name=\"edit1\" id=\"edit1\" rows=\""
          . "30\" cols=\"200\" STYLE=\"font-family:Arial;font-size:9pt;\">"
          . $BD
          . "</textarea><br></html>" );
    return;
}
...
sub MSwitch_FullBackup(@) {
	...
	my $MONKEYS = join( "|", keys(%MONATE) );
				FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('- $newtime')", "");
	...
	if ($arg ne "save")
		{
		FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('- schreibe Daten für $testdevice')", "");
		}
		...
		if ($arg eq "delete")
		{
			...
			FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('- delete $testdevice')", "");
		}
    }
	...
	if ($arg eq "save")
			{
				...
				print BACKUPDATEI "$BD";
				close(BACKUPDATEI);
				MSwitch_LOG( $Name, 1, "$Name delayed Shutdown: ".$restoredir."MSwitch_Save.txt wurde geschrieben");
				return;
			}
			else
			{
				open( BACKUPDATEI, ">".$restoredir."MSwitch_Full_".$newtime.".txt" );
				print BACKUPDATEI "$BD";
				close(BACKUPDATEI);
				
				FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information(' ')", "");
				FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('############################################################################')", "");
				FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('Backupdatei fhem/".$restoredir."MSwitch_Full_$newtime wurde geschrieben.')", "");
				FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('############################################################################')", "");
...
sub MSwitch_Get_Backup_inhalt(@) {
	...
	my $string = $arg1;
	my $Zeilen="";
    open( BACKUPDATEI, "<./".$restoredir.$string ) || return "no Backupfile found!\n";
    while (<BACKUPDATEI>) 
	{
        $Zeilen = $Zeilen . $_;
    }
...
sub MSwitch_fullrestorelocal(@) {
	...
	if ($string eq "makefile"){
		FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information(' ')", "");
		FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('Datei wird auf Server angelegt.')", "");
...
sub MSwitch_fullrestore(@) {
	...
	{
		FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('modus: Einzelrestore')", "");
		$mode ="einzel";
	}
	else
	{
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('modus: Fullrestore')", "");
	$data{MSwitch}{runningbackup}="ja";
	# ÄNDERN nur wenn device vorhanden !
    AnalyzeCommand($hash, "set TYPE=alexa stop");
	$mode ="full";
	
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('Das System wird für den Restore vorbereitet.')", "");
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('Aus Sicherheitsgünden werden verschiedene Dienste gestoppt')", "");
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information(' ')", "");
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('MSwitch-Systemvorbereitung: Notifyverarbeitung gestoppt.')", "");
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('MSwitch-Systemvorbereitung: Init-Check gestoppt.')", "");
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('MSwitch-Systemvorbereitung: Load-helper gestoppt.')", "");
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('MSwitch-Systemvorbereitung: Alexadevices gestoppt.')", "");
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information(' ')", "");
	}
	...
	foreach (@found) 
		{	
			if ( $_ =~ m/#N -> (.*)/ )    # setreading
			{
					...
					$Zeilen =~ s/#\[nl\]/\n/g;
					$aktname=$Zeilen;
					my $error = AnalyzeCommand($hash, "delete ".$aktname);
					my $error1 = AnalyzeCommand($hash, "define ".$aktname." mswitch");
					
					if ( defined($error1) )
						{
							$encoded = urlEncode($error1);
							FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('!!! Fehler aufgetreten: $encoded')", "");
							$data{MSwitch}{runningbackuperror}.="\n - $error1";
						}
	...
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('Die Devices wurden erstellt, beginne Datenimport ...')", "");
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information(' ')", "");
...
sub MSwitch_fullrestore_end($) {
	...
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('CLEARSCREEN')", "");
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('###################################################')", "");
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('Restore beendet.')", "");
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('Achtung , das MSwitch-Modul befindet sich in einem Restoremodus und ist nicht funktionsfähig.')", "");
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('Bitte ein fhem.save dürchführen und Fhem neu starten ')", "");
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information(' ')", "");
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('es wurden $anzdevices Devices wiederhergestellt')", "");
	
	foreach (@devices) 
			{
			FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information(' - $_')", "");
			}

	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information(' ')", "");
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('Folgende Fehler sind aufgetreten: ')", "");
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('$encoded')", "");
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information(' ')", "");
	FW_directNotify( "FILTER=$Name","#FHEMWEB:WEB", "information('###################################################')", "");
	...
	if(!exists $hash->{helper}{delaydetails}{$timecondition}{cmd})
		{
			...
			MSwitch_LOG( $name, 5,  "delaydeteils: ".$hash->{helper}{delaydetails}{$timecondition});
...
if (  !defined $devicedetails{ $device . '_repeatcount' }){$devicedetails{ $device . '_repeatcount' } =0};
if (  !defined $devicedetails{ $device . '_repeattime' }){$devicedetails{ $device . '_repeattime' } =0};
...
sub MSwitch_EventBulk($$$$) {
	...
	readingsBulkUpdate( $hash, "EVTPART1",$evtparts1);
	readingsBulkUpdate( $hash, "EVTPART2", $evtparts2);
	readingsBulkUpdate( $hash, "EVTPART3", $evtparts3 );
	
	readingsEndUpdate($hash, $update);
	
	#MSwitch_LOG( $name, 7,  "update $update" );
...
#  ändern des loglevels per cmd
# {$data{MSwitch}{perlteste}{loglevel}=7}
...
sub MSwitch_makefreecmdonly($$) {
	...
	MSwitch_LOG($name, 6, "TESTFREECMD (only] -> cs -> -$cs-" );
...
sub MSwitch_makefreecmd($$) {
	...
	MSwitch_LOG($name, 6, "TESTFREECMD  -> cs -> -$cs-" );
...
sub MSwitch_gettemplate($$) {
    ...
    if ( $template =~ m/\.*\/(.*)/s ) {
        ...
        open( BACKUPDATEI, "<$adress" )
          || MSwitch_LOG( "test", 0, "ERROR " . $adress );
        while (<BACKUPDATEI>) {
            $data = $data . $_;
        }
        close(BACKUPDATEI);
        ...
        if ( my @matches = $data =~ /$regex/sg )
		{
            ...
            if ( $mapss ne "no_mapping" )
			{
                foreach my $mapping (@maps)
				{
					...
					#MSwitch_LOG $name, 6, "$org newSign -$ers-";
...
sub MSwitch_PerformGetRequest($$) {
    ...
    my @maps;
    @maps = split( /\n/, $mapss );
    my $regex = AttrVal( $name, "MSwitch_ExtraktfromHTTP", "FullGETResponse->(.*)" );
    my @gset = split( /\n/, $regex );
...
sub X_ParseHttpResponse($) {
    ...
    {
        Log3 $name, 1,"url " . $param->{url} . " returned: $data";    # Eintrag fürs Log
...
sub MSwitch_Countdown_new(@) 
{
	...
	# MSwitch_LOG( $name, 7, " --------- incomming oldtime: ".$oldtime);
	...
	foreach my $countdown ( keys %{$hash->{helper}{countdown}} )
		{		 		
			$hash->{helper}{countdown}{$countdown}=$hash->{helper}{countdown}{$countdown}-$jump;		
			if ($hash->{helper}{countdown}{$countdown} <=0)
				...
				$format =~ s/SS/00/g;	
				$format =~ s/ss/0/g;	
				readingsBulkUpdate( $hash, $countdown , $format);
				next;
				}
