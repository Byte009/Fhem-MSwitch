...
sub MSwitch_LoadHelper($) {
    ...
    ##### MSwitch_LOG( $Name, 0,"->$Name Loadhelper  ");
    ##### MSwitch_LOG( $Name, 0,"->$Name notdef start:".$hash->{NOTIFYDEV} );
    ...
    if ( defined $hash->{DEF} ) {
        ...
        if ( defined $devhash ) {
            ##### MSwitch_LOG( $Name, 0,"->$Name cdev $cdev  ");
...
################
    ...
    ##### MSwitch_LOG( $Name, 0,"->$Name end notdef:".$hash->{NOTIFYDEV} );
    return;
}
          ...
          )
        {
            ...
            if ( $configtype eq "MSwitch" ) {
                ...
                foreach my $key (@areadings) {
                    next
                      if ( $key !~
m/(^MSwitch_.*|stateFormat|devStateIcon|icon|room|disable)/
                      );
                    next
    ...
    if ( $opt eq 'extcmd' ) {
        ...
        if ( $typ ne "FreeCmd" ) {
            ...
            if ( $cs =~ m/(\{)(.*)(\})/s ) {
                ...
                $cs =~ s/}$/}<br>/g;
                return
                  "<small>$EXECUTEDCMD:</small><br><br>$cs<br><br>$errorout";
                return "ok";
            }
            else {
                #fhemmode
                ...
                $exec =~ s/}$/}<br>/g;
                return
"<small>$EXECUTEDCMD (Fhemmode):</small><br><br>$exec<br><br>$errorout";
            }
        }
        return;
    }
...
####################
    if ( $opt eq 'checkcondition' ) {
        ...
        ##### MSwitch_LOG( $name, 6, "aufruf condcheck ############# " . __LINE__ );
...
sub MSwitch_Set_ChangeRenamed($@) {
    ...
    if ( @found_devices > 0 ) {
        FW_directNotify(
            ...
            "#FHEMWEB:WEB",
            "information('Device \"$name\" angepasst ($args[0]->$args[1])...')",
            ""
        );
    }
    return;
...
sub MSwitch_Set_SetTrigger($@) {
    ...
    if ( defined $args[1] && $args[1] ne 'NoTimer' ) {
        readingsSingleUpdate( $hash, ".Trigger_time_1", $args[1], 0 );
    }
    if ( defined $args[2] && $args[2] ne 'NoTimer' ) {
        readingsSingleUpdate( $hash, ".Trigger_time_2", $args[2], 0 );
    }
    if ( defined $args[3] && $args[3] ne 'NoTimer' ) {
        readingsSingleUpdate( $hash, ".Trigger_time_3", $args[3], 0 );
    }
    if ( defined $args[4] && $args[4] ne 'NoTimer' ) {
        readingsSingleUpdate( $hash, ".Trigger_time_4", $args[4], 0 );
    }
    if ( defined $args[5] && $args[5] ne 'NoTimer' ) {
        readingsSingleUpdate( $hash, ".Trigger_time_5", $args[5], 0 );
    }
...
sub MSwitch_Set_OnOff($@) {
    ...
    ##### MSwitch_LOG( $name, 6, "EINGANG SETONOFF \n $ic,$showevents,$devicemode,$delaymode,$hash, $name, $cmd, @args " );
...
##### MSwitch_LOG( $name, 6,"dummyfound set data".__LINE__);
    ...
    ##### MSwitch_LOG( $name, 6,"### Aufruf SUB_Exec_Notif mit event : $event ". __LINE__ );
...
#################################
    ...
    if ( $cmd eq 'fullrestore' ) {
        my $ret = MSwitch_fullrestore( $hash, $args[0] );
        return $ret;
    }
...
# setze eingehendes Event :
        ...
        MSwitch_LOG( $ownName, 6, "-> eingehendes Event: $eventcopy ");
    ...
    if ( AttrVal( $ownName, 'MSwitch_Eventhistory_to_Reading', "0" ) == 1 ) {
        ...
        foreach my $testdevices (@eventfunction) {
            ...
            if ( ReadingsVal( $ownName, ".Trigger_device", "no_trigger" ) eq
                'all_events' )
            {
                readingsBulkUpdate( $own_hash, $devName . "_" . $readname,
                    $testdevices );
                readingsBulkUpdate(
                    $own_hash,
                    $devName . "_" . $readnametime,
                    $eventfunctiontime[$count]
                ) if $timestampreading == 1;
                readingsBulkUpdate( $own_hash,
                    $devName . "_" . $readnamerealtime, $realtime )
                  if $realtimerreading == 1;

            }
            else {
                readingsBulkUpdate( $own_hash, $readname, $testdevices );
                readingsBulkUpdate( $own_hash, $readnametime,
                    $eventfunctiontime[$count] )
                  if $timestampreading == 1;
                readingsBulkUpdate( $own_hash, $readnamerealtime, $realtime )
                  if $realtimerreading == 1;
...
################
  ...
  if ($tmswitchsetlist ne "undef"){
			...
			#MSwitch_LOG( $Name, 0,"dynsetlist @testdynsetlist"); 
...
#MSwitch_LOG( $Name, 0,"testreadingarg $testreadingarg"); 
#MSwitch_LOG( $Name, 0,"tsetreading $tsetreading"); 

$data{MSwitch}{mssetlist}{$tsetreading} = $testreadingarg;

#MSwitch_LOG( $Name, 0,"data{MSwitch}{mssetlist}{$tsetreading} = $testreadingarg");

#if ( $data{MSwitch}{mssetlist}{$tsetreading}) delete
                }
			}      
		}
...
MSwitch_LOG( $name, 6,"\n----------  SUB Exec_Notif ----------\n->  event: $event\n-> comand: $comand");
    ...
    my %devicedetails = MSwitch_makeCmdHash($name);
    return
      if ReadingsVal( $name, '.Device_Affected', 'no_device' ) eq 'no_device';
                    ...
                    if ( $expertmode eq '1' ) {
                            ...
                            && $devicedetails{ $device . $field } ne "" )
                        {
                            ...
                            readingsBulkUpdate( $hash,
                                "lastsetting_delay_ident", $savename );
                            readingsBulkUpdate( $hash, "lastsetting_delay_cmd",
                                $data{MSwitch}{$name}{setdata}{last_cmd} );
                            readingsBulkUpdate( $hash,
                                "lastsetting_delay_time", $format );
...
##############################################################
 ...
 # if ( $device =~ m/{.*}/ ) {
           if ( $device =~ m/^{.*}$/ ) {
                ...
                my $deepsafe = "undef";
                $deepsafe = "set $name " . $hash->{helper}{deepsave}
                  if exists $hash->{helper}{deepsave};
...
"$Name-> Timer blockiert, NOTIFYDEV deaktiviert - Versionskonflikt L:"
    ...
    readingsBulkUpdate( $hash, "EVENT",
        $Name . ":execute_timer_P" . $param . ":" . $extime, $showevents );
...
sub MSwitch_Delete_Delay($$) {
    ...
    if ( $device eq 'all' ) {
        ...
        foreach my $countdown ( keys %{ $hash->{helper}{countdown} } ) {
			...
			#$showevents = MSwitch_checkselectedevent( $hash, $countdown );
...
sub MSwitch_VersionUpdate($) {
    ...
    my $message = "";
    $message .=
      "MSwitch-Strukturupdate -> Autoupdate fuer MSwitch_Device $Name";
...
"information('MSwitch-Systemvorbereitung: Notifyverarbeitung gestoppt.')",
    ...
    foreach (@found) {
        if ( $_ =~ m/#N -> (.*)/ )    # setreading
        {
            ...
            my $error1 =
              AnalyzeCommand( $hash, "define " . $aktname . " mswitch" );
...
#MSwitch_LOG( $name, 6,"------- $todec >");
    ...
    #variablen etc.
    # $we / !$we 	 - 	nur an Wochenenden / Wochentagen
    # isday / !isday 	 - 	Tag / nicht Tag
    # $year 	 - 	Jahr
    # $month 	 - 	Monat (1-12)
    # $day 	 - 	Tag des Monats (1-30/31)
    # $wday 	 - 	Wochentag (1-7)
    # $yday 	 - 	Kalendertag (1-356)
    # $hms 	 - 	aktuelle Zeit (hh:mm:ss)
    # $timestamp 	 - 	aktuelle Zeit als Zeitstempel (unix)
    # $SELF 	 - 	enthält den eigenen Namen

    #$ARG argument aus timer
...
sub MSwitch_reloadreadings($$) {
    ...
    my $testreading = $devhash->{READINGS};
    my @areadings =
      ( keys %{$testreading} );    # enthält alle readings des devices
