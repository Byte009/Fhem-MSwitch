...
####################
    if ( $opt eq 'checkcondition' ) {
		...
		#MSwitch_LOG( $name, 0, "condsplit: " . $condsplit );
        ...
        # while ( $condsplit =~ m/(if \()(.*)(\()(.*')(.*)',\s'(.*)',\s(.*)/ ) {
            ...
            # $reads .= "<div style=\"color: #FF0000\">" . $NOREADING . "</div>"
		...
		##if (ReadingsVal('MSwitch_Updater', 'send_info', 'undef') eq "on" && (AttrVal('MSwitch_Updater', 'Telebot', 'nobot')) ne "nobot"){$answer = 'true';} else {$answer = 'false';} 
		
		 while ( $condsplit =~
m/(.*)(\()(ReadingsVal|ReadingsNum|ReadingsAge|AttrVal|InternalVal)\((')(.*)('),.(')(.*)('),.(')(.*)(')\)(.*)/) {
     
	 # MSwitch_LOG( $name, 0, "1: " . $1 );
	 # MSwitch_LOG( $name, 0, "2: " . $2 );
	 # MSwitch_LOG( $name, 0, "3: " . $3 );
	 # MSwitch_LOG( $name, 0, "4: " . $4 );
	 # MSwitch_LOG( $name, 0, "5: " . $5 );
	 # MSwitch_LOG( $name, 0, "6: " . $6 );
	 # MSwitch_LOG( $name, 0, "7: " . $7 );
	 # MSwitch_LOG( $name, 0, "8: " . $8 );
	 # MSwitch_LOG( $name, 0, "9: " . $9 );
	 # MSwitch_LOG( $name, 0, "10: " . $10 );
	 # MSwitch_LOG( $name, 0, "11: " . $11 );
	 # MSwitch_LOG( $name, 0, "12: " . $12 );
	 # MSwitch_LOG( $name, 0, "13: " . $13 );
	 
	 my $readname = $3.": [$5:$8] - ";
	 my $readincmd = "$3( '$5', '$8', '$11' )";
	 my $readinginhalt = eval $readincmd;
	 
	 # MSwitch_LOG( $name, 0, "readname: " . $readname );
	 # MSwitch_LOG( $name, 0, "readincmd: " . $readincmd );
	 # MSwitch_LOG( $name, 0, "readinginhalt: " . $readinginhalt );
	 # MSwitch_LOG( $name, 0, "condsplittmp: " . $condsplit );
	$condsplit = $1 .  $13;
	$x++;    # exit
	last if $x > 20;
	$reads .=$readname ."     ". $INHALT . " ". $readinginhalt . "<br>";
 }
	
	
	
	#MSwitch_LOG( $name, 0, "condsplitout: " . $condsplit );
...
### modulmode - no sets
 ...
 {
  return
"Unknown argument $opt, choose one of config:noArg support_info:noArg restore_MSwitch_Data:this_Device active_timer:show,delete";
...
##########################
    ...
    if ( $cmd ne "?" ) {
        ...
        MSwitch_LOG( $name, 6, "----------------------------------------" );
        MSwitch_LOG( $name, 6, "eingehender Befehl: Set $cmd, @args " );
    }
...
##############################
        ...
        if ( $cmd ne "?" ) {
            ...
            if ( @arraydynsetlist && grep /$re/, @arraydynsetlist ) {
                ...
                #MSwitch_LOG( "DEBUG", 0,"@gefischt" . __LINE__ );

#my @rl = split( " ", AttrVal( $name, "readingList", "" ) );
# if (( @rl && grep /$re/, @rl ) || ( @arraydynsetlist && grep /$re/, @arraydynreadinglist ))
#{
                readingsSingleUpdate( $hash, $cmd, "@args", 1 );
...
"Unknown argument $cmd, choose one of $dynsetlist reset_device:noArg active:noArg inactive:noArg del_function_data:noArg del_delays:noArg backup_MSwitch:all_devices fakeevent exec_cmd_1 exec_cmd_2 wait reload_timer:noArg del_repeats:noArg change_renamed reset_cmd_count:1,2,all $setList $special";
            ...
            return
"Unknown argument $cmd, choose one of $dynsetlist reset_device:noArg active:noArg del_function_data:noArg inactive:noArg on off del_delays:noArg backup_MSwitch:all_devices fakeevent wait reload_timer:noArg del_repeats:noArg change_renamed $setList $special";
        }
...
##############################
    if ( $cmd eq 'reset_device' ) {
        if ( $args[0] eq 'checked' ) {
            ...
            #delete( $hash->{helper}{devicecmds1} ); #löschen
            #delete( $hash->{helper}{last_devicecmd_save} ); #löschen
			...
			#delete $defs{$name}{.eventMapCmd};
			#fhem ("delete($defs{$Name}{.eventMapCmd})");
            # attribute
            my %keys;
...
##############################
    if ( $cmd eq "devices" ) {
        ...
        my @olddevices = split(
                                /,/,
                                ReadingsVal(
                                          $name, '.Device_Affected', 'no_device'
                                )
        );
...
###################################################
        ...
        MSwitch_LOG( $name, 5, "On/Off Kommando gefunden -> $cmd " . __LINE__ );
...
#	MSwitch_LOG( $name, 6,"Teste auf Schaltbedingungen -> keine vorhanden" ." "  . __LINE__ );
            ...
            {
#      MSwitch_LOG($name,6,"Zufallstimer gefunden -> ". $devicedetails{$timerkey} ." "  . __LINE__);
...
MSwitch_LOG( $name, 0,"zurück von freecmd: -> " . __LINE__ );
                }
...
#     MSwitch_LOG( $name, 6,"Teste auf Verzögerung -> " . $devicedetails{$timerkey}." " . __LINE__  );
                     ...
                     || $devicedetails{$timerkey} eq "" )
                {
                    ...
                    MSwitch_LOG(
                                 $name,
                                 6,
                                 "Schaltbedingung nicht vorhanden:"
                                   . $devicedetails{$conditionkey} . " "
                                   . __LINE__
...
#MSwitch_LOG( $name,0,"cs -> ".$cs." ". __LINE__);
...
#			MSwitch_LOG( $name, 6,"Abbruchbefehl erhalten von ". $device  ." "  . __LINE__);
        ...
        MSwitch_LOG(
                     $name,
                     6,
                     "Anzahl der auszufuehrenden befehle -> "
                       . $anzahl . " "
                       . __LINE__
        );
        MSwitch_LOG( $name, 6, "Übergabe an Execute erfolgt" . " " . __LINE__ )
          if $anzahl > 0;
...
sub MSwitch_Cmd(@) {
    ...
    foreach my $cmds (@cmdpool) {
        MSwitch_LOG( $Name, 6,
                     "Befehlsausfuehrung -> " . $cmds . " " . __LINE__ );
        my @cut = split( /\|/, $cmds );
        $cmds = $cut[0];
        ...
        if ( $cmds =~ m/set (.*)(MSwitchtoggle)(.*)/ ) {
            MSwitch_LOG( $Name, 6,
                         "Togglemode erkannt -> " . $cmds . " " . __LINE__ );
            $toggle = $cmds;
             ...
             && $devicedetails{ $device . '_repeatcount' } ne '' )
        {
            MSwitch_LOG(
                         $Name,
                         6,
                         "teste auf Befehlswiederholungen -> "
            ...
            MSwitch_LOG(
                         $Name,
                         6,
                         "Befehlswiederholungen nach SETMAGICersetzung -> "
                           . $devicedetails{ $device . '_repeatcount' } . " "
                           . __LINE__
            );
        }
             ...
             && $devicedetails{ $device . '_repeattime' } ne '' )
        {
            MSwitch_LOG(
                         $Name,
                         6,
                         "Teste auf Wiederholungsintervall -> "
            ...
            MSwitch_LOG(
                         $Name,
                         6,
                         "Wiederholungsintervall nach SETMAGIcersetzung -> "
                           . $devicedetails{ $device . '_repeattime' } . " "
                           . __LINE__
            );
        }
...
############################
        ...
        if ( AttrVal( $Name, 'MSwitch_Debug', "0" ) eq '2' ) {
            ...
            if ( $cmds =~ m/({)(.*)(})/ ) 
				{
					my $out = eval($cmds);
					if ($@) { MSwitch_LOG( $Name, 0,"MSwitch_Set: ERROR $cmds: $@ " . __LINE__ ); }
				}
            else{
...
sub MSwitch_toggle($$) {
    ...
    MSwitch_LOG( $Name, 6,
                 "########## Togglefunktion ende return: $newcomand" );
    return $newcomand;
}
...
##################################
	...
	foreach my $line (@gset) 
	{
		...
		#Log3( $name, 0, $lineset[0]." - ".$lineset[1]);
    ...
    #
    foreach my $key (@areadings) {
        	#Log3( $name, 0, "key - ". $key ." Inhalt ".$data{MSwitch}{$name}{groups}{$key});
    }
...
##### test wait attribut
    ...
    MSwitch_LOG( $ownName, 6, "$ownName: --- TESTE WAIT ---" );
    if ( ReadingsVal( $ownName, "waiting", '0' ) > time ) {
        MSwitch_LOG(
                     $ownName,
                     6,
                     '$ownName: AKTION DURCH WAIT ABGEBROCHEN -> '
        ...
        MSwitch_LOG( $ownName, 6,
               "$ownName: WAIT -> " . ReadingsVal( $ownName, "waiting", '0' ) );

        # reading löschen
        delete( $own_hash->{READINGS}{waiting} );
    }
    MSwitch_LOG( $ownName, 6, "$ownName: --- WAIT PASSIERT ---" );
...
##########################
    ...
    # kann ggf an den anfang der routine gesetzt werden ? test erforderlich
           ...
           # fügt dem event den devicenamen hinzu , wenn global getriggert wird
                ...
                MSwitch_LOG( $ownName, 6,
                            "$ownName: teste die eventcopy -> " . $eventcopy1 );
           ...
           # fügt dem event den devicenamen hinzu , wenn global getriggert wird
                ...
                foreach my $filter (@filters) {
                    ...
                    if ( $eventcopy1 =~ m/$filter/ ) {
                        MSwitch_LOG( $ownName, 6,
"Name: eingehendes Event durch MSwitch_Trigger_Filter ausgefiltert: "
                              . $eventcopy1 );
                        next EVENT;
                    }
                }
...
##########################################
                ...
                #push @cmdarray, $own_hash . ",$zweig,nocheck,$bridge";
                #$check     = 1;
                #$foundcmdbridge = 1;
      ...
      #next EVENT if $chbridge ne "no_bridge";
       ...
       # next EVENT if $chbridge ne "no_bridge";
            ...
            if ( $triggercmdon ne 'no_trigger' ) {
                MSwitch_LOG( $ownName, 6,
                             "$ownName: checktrigger trigger cmd-3 -> " );
                $testvar =
                  MSwitch_checktrigger(
                                        $own_hash,        $ownName,
...
"$ownName MSwitch_Notify: Fehler bei Befehlsausführung $errors -> Comand: $_ "
        ...
        MSwitch_LOG( $ownName, 6, "FOUNDCMDBRIDGE:$foundcmdbridge" );
     ...
     #schaltet zweig 3 und 4
        ...
        if ( $anzahl && $anzahl != 0 ) {
          ...
          LOOP31: foreach (@cmdarray) {
                ...
                MSwitch_LOG( $ownName, 6, "$ownName: Befehl -> " . $test );
                my ( $ar1, $ar2, $ar3, $ar4 ) = split( /,/, $test );

                MSwitch_LOG( $ownName, 6, "$ownName: ar1 $ar1" );
                MSwitch_LOG( $ownName, 6, "$ownName: ar2 $ar2" );
                MSwitch_LOG( $ownName, 6, "$ownName: ar3 $ar3" );
                MSwitch_LOG( $ownName, 6, "$ownName: ar4 $ar4" );

                if ( !defined $ar2 ) { $ar2 = ''; }
                if ( $ar2 eq '' ) {
                    MSwitch_LOG( $ownName, 6, "$ownName: NEXTLOOP" );
                    next LOOP31;
                }
                my $returncmd = 'undef';
                $returncmd =
...
"--------------------------------------------------------------------------------"
    ...
    MSwitch_LOG( $name, 5, "PRUEFE SCHLUESSEL" );
    my $foundkey = "undef";
    ...
    if ( !defined $hash->{helper}{eventtoid}{$foundkey} ) {
        ...
        #	MSwitch_LOG( $name, 6, "NO BRIDGE FOUND !" );
...
#	MSwitch_LOG( $name, 6, "HELPER EVENT TO ID : " . $hash->{helper}{eventtoid}{$foundkey} );
    ...
    #   MSwitch_LOG( $name, 6, "DISTRIBUTOROUT: $bridge[2] " );
    MSwitch_LOG( $name, 6, "BRIDGE FOUND !" );
...
####################  nur bei entsprechender regex
        ...
        if ( $test =~ m/(.*)\((.*)\)(.*)/ ) {
            ...
            # if ( index( $_, '*', 0 ) == -1 ) {
				...
				# # ändern alle evnets + verwendete events - doppelte löschen
...
$errors .= ' ' . '[FREECMD]';
	...
	#Log3( $Name, 0, "affecteddevices - ". $affecteddevices );
	
    foreach my $key (@areadings) {
        	#Log3( $Name, 0, "key - ". $key ." Inhalt ".$data{MSwitch}{$Name}{groups}{$key});
   
	 #$select = index( $affecteddevices, "[MSwitchGruppe]_".$key."-AbsCmd1", 0 );
	 my $fullname = $key;
	 
	 #@deftoarray
	  #Log3( $Name, 0, "deftoarray - @deftoarray ". @deftoarray);
	 #Log3( $Name, 0, "fullname - ". $fullname);
	 ...
	 #Log3( $Name, 0, "test - @test anzahl: ". @test);
     $selected = "";
	 if ( @test > 0 ) 
	 { 
	 $selected = "selected=\"selected\" " ;
	 }
	#Log3( $Name, 0, "selected -". $selected);
...
#Log3( $Name, 0, "msgruppen - @msgruppen ". @msgruppen);
#Log3( $Name, 0, "devicenamet -". $devicenamet);
#Log3( $Name, 0, "grep devicenamet - ". grep (/$re/,@msgruppen));


			if ( grep(/$re/,@msgruppen) > 0) {
			
				#Log3( $Name, 0, "erzeuge cmdsatz für -". $devicenamet);
MSwitch_makegroupcmd($hash,$devicenamet);
                $cmdsatz{$devicenamet} = '[FREECMD]';
            }
...
if ( grep(/$re/,@msgruppen) > 0) {
$groupbutton="<input type='button' id='' value='showdevices' onclick=\"javascript:showgroup('".$devicenamet."')\";>";

}


            if ( $expertmode eq '1' ) {
                $NAMEsatz ="$zusatz $devicenamet $realname&nbsp;&nbsp;$groupbutton&nbsp;&nbsp;$dalias $alert";
...
for (@readlist) {
	   ...
	   $inhalt .= "<tr class=\"".$oddeven."\"><td><div class=\"dname\" data-name=\"".$Name."\">".$_."</div></td>\\
<td><div class=\"dval\" informid=\"".$Name."-".$_."\">".$setinhalt."</div></td>\\
...
# my $inhalt = "<tr class=\"odd\"><td><div class=\"dname\" data-name=\"trt\">EVENT</div></td>\\
# <td><div class=\"dval\" informid=\"trt-EVENT\">Updatebenachrichtigung:execute_timer_P3:12:00</div></td>\\
# <td><div informid=\"trt-EVENT-ts\">2020-04-19 12:00:00</div></td>\\
# </tr>";
...
// \$(\".makeTable.wide.readings\").css('display','none');
	...
	# \$(\".makeTable.wide.readings\").attr(\"id\",\"mwr\");
	 
	# \$(\"#mwi\").attr('class', 'makeTable wide readings')
	# \$(\"#mwr\").attr('class', 'makeTable wide internals')
...
"--------------------------------------------------------------------------------"
    ...
    my $testtoggle = '';
    MSwitch_LOG( $name, 6,
        "Zu schaltende devices -> " . @devices . " @devices" . " " . __LINE__ );
...
#MSwitch_LOG( $name, 6,"Timer -> " . $devicedetails{$timerkey} ." "  . __LINE__ );
        ...
        MSwitch_LOG( $name, 6,
              "$name: TIMER -> " . $devicedetails{$timerkey} . " " . __LINE__ );
        ...
        if ( $devicedetails{$timerkey} =~ m/[\d]{2}:[\d]{2}:[\d]{2}/ ) {
            MSwitch_LOG( $name, 6, "$name: TIMERFORMAT OK " . " " . __LINE__ );
            my $hdel = ( substr( $devicedetails{$timerkey}, 0, 2 ) ) * 3600;
            my $mdel = ( substr( $devicedetails{$timerkey}, 3, 2 ) ) * 60;
            my $sdel = ( substr( $devicedetails{$timerkey}, 6, 2 ) ) * 1;
            $devicedetails{$timerkey} = $hdel + $mdel + $sdel;
...
# MSwitch_LOG( $name,6, "Timer des devices -> " . $devicedetails{$timerkey}  ." "  . __LINE__);
        ...
        if ( $devicedetails{$key} ne "" && $devicedetails{$key} ne "no_action" )
        {
			...
			#MSwitch_LOG( $name, 0, "zurück in execnotif nach MSwitch_makefreecmd " );
...
# MSwitch_LOG( $name, 0, "HIER " );
           ...
           # $cs =~ s/\$SELF/$name/;
                 ...
                 || $devicedetails{$timerkey} eq "" )
            {
                ...
                if ( $execute eq 'true' ) {
                    ...
                    MSwitch_LOG( $name, 6,
                              "$name: TESTE WIEDERHOLUNGEN " . " " . __LINE__ );
                    my $x = 0;
                    while ( $devicedetails{ $device . '_repeatcount' } =~
                            m/\[(.*)\:(.*)\]/ )
                    {
                    ...
                    MSwitch_LOG(
                                 $name,
                                 6,
                                 "$name: INTERVALL DER WIEDERHOLUNGEN -> "
                                   . $devicedetails{ $device . '_repeattime' }
                                   . " "
                                   . __LINE__
                    );
					...
					#MSwitch_LOG( $name, 0, "nach mswitch_dec: " . $cs );
				...
				# MSwitch_LOG( $name, 0, "cs unmittelbar vor ausführung: " . $cs );
           ...
           # MSwitch_LOG( $name, 0,"AUSFÜHRUNG BEFEHL" . __LINE__);
            #MSwitch_LOG( $name, 0,"AUSFÜHRUNG BEFEHL INHALT $cs " . __LINE__);
                            ...
                            if ( defined($errors) and $errors ne "OK" ) {
                                MSwitch_LOG( $name, 6,
"MSwitch_Exec_Notif $comand: ERROR $device: $errors -> Comand: $cs"
                                      . " "
                                      . __LINE__ );
                            }
                            ...
                            MSwitch_LOG( $name, 6,
                                  "$name: AUSFÜHRUNG BEFEHL ENDE" . __LINE__ );
                        }
                    }
...
"--------------------------------------------------------------------------------"
    ...
    MSwitch_LOG( $name, 6, "AUSFÜHRUNG VERZÖGERTER BEFEHL (DELAY)" );
    MSwitch_LOG(
        $name,
        6,
"--------------------------------------------------------------------------------"
    ...
    MSwitch_LOG( $name, 5, $incomming . " " . __LINE__ );
    ...
    my $toggle = '';
    if ( $execute eq 'true' ) {
        MSwitch_LOG( $name, 6, "$name: BEFEHLSAUSFÜHRUNG -> $cs " . __LINE__ );
...
######## unterscheidung der funktionen
            ...
            if ( $funktionsstringdiff =~ m/(DIFF)(.*)/ ) {
                ...
                my $vergleichswert = $2;
                MSwitch_LOG( $name, 5,
                             "$name:     Funktion ist:  - $function" );
                MSwitch_LOG( $name, 5,
                             "$name:     index ist:  - $vergloperand" );
                MSwitch_LOG( $name, 5,
...
"Funktionberechnung AVERAGE<br>Berechnung nicht möglich, nicht genug Daten vorhanden";
                    ...
                    MSwitch_LOG( $name, 5,
                                 "$name: Teststring ergebniss: $erg1" );
...
#MSwitch_LOG( $name, 0, "found condition: ".$condition ) if $name eq "MSwitch_Updater";
...
#MSwitch_LOG( $name, 0, "firstpart: " . $firstpart );
	
#MSwitch_LOG( $name, 0, "readingtyp: " . $readingtyp );
#MSwitch_LOG( $name, 0, "readingdevice: " . $readingdevice );
#MSwitch_LOG( $name, 0, "readingname: " . $readingname );
#MSwitch_LOG( $name, 0, "readingstandart: " . $readingstandart );
#MSwitch_LOG( $name, 0, "lastpart: " . $lastpart );
        ...
        # $reading = ReadingsVal( $readingdevice, $readingname, $readingstandart )
          # if $readingtyp eq "ReadingsVal";
        # $reading = ReadingsNum( $readingdevice, $readingname, $readingstandart )
          # if $readingtyp eq "ReadingsNum";
        # $reading = ReadingsAge( $readingdevice, $readingname, $readingstandart )
          # if $readingtyp eq "ReadingsAge";
        # $reading = AttrVal( $readingdevice, $readingname, $readingstandart )
          # if $readingtyp eq "AttrVal";
        # $reading = InternalVal( $readingdevice, $readingname, $readingstandart )
          # if $readingtyp eq "InternalVal";
		  
	
		  
		  
		  
		my $code ="(".$readingtyp."('".$readingdevice."', '".$readingname."', '".$readingstandart."'))"  ;
...
#MSwitch_LOG( $name, 0, "found condition out: " . $condition )  if $name eq "MSwitch_Updater";
...
#$condition =~ s/(?<!\!)=(?!~)/==/ig; #https://www.dev-insider.de/regex-zum-suchen-und-ersetzen-nutzen-a-840347/
  ...
  END:
    ...
    if ( $debugging eq "1" ) {
        MSwitch_LOG( "Debug", 0, "eveal line" . __LINE__ );
        MSwitch_LOG( $name,   0, "$name: finalstring -> " . $finalstring );
    }
...
sub MSwitch_Checkcond_history($$) {
	...
	#MSwitch_LOG( $name, 0, "$condition" );

	
	#MSwitch_LOG( $name, 0, $hash->{helper}{history}{eventberechnung} );
	...
	#MSwitch_LOG( $name, 0, "split: @historysplit ".@historysplit );
	...
	# if ( @historysplit == 3){
	# $historynumber = $historysplit[2];
	# }
	# elsif ( @historysplit == 3){
	...
	#MSwitch_LOG( $name, 0, "numberf $historynumber" );
...
sub MSwitch_dec($$) {
	...
	MSwitch_LOG( $name, 5, "vor MSwitch_dec: " . $todec );
	...
	my @toswitch = (split (/ /,$todec));
	
	my $nametoswitch = $toswitch[1];
	MSwitch_LOG( $name, 5, "nametoswitch: " . $nametoswitch );
	...
	my $retest            = qr/$nametoswitch/;
	
	Log3( $name, 5, "retest - $retest");
	Log3( $name, 5, "msgruppen - @msgruppen");
	Log3( $name, 5, "greptest - ".grep (/$retest/,@msgruppen));
	
	
	if ( grep (/$retest/,@msgruppen) > -1) { #nur wenn entsprechender gruppenname vorhanden ist 
			
				Log3( $name, 5, "gruppenname gefunden - $nametoswitch");
				my $newdevlist = $data{MSwitch}{$name}{groups}{$nametoswitch};
				
				Log3( $name, 5, "newdevlist gefunden - $newdevlist");
				
				$toswitch[1] = $newdevlist;
				
				$todec = join(' ',@toswitch);
				
				
				$todec =~ s/( {2,10})/ /g;
				#$s=~y/!-}/!-}/s; # Alles von ! bis }  ( {2,10})
	...
	###########################################################################
	MSwitch_LOG( $name, 5, "nach MSwitch_dec: " . $todec );
    return $todec;
}
...
#$cs =~ s/\n//g;
	...
	if ( $cs =~ m/({)(.*)(})/s ) {
		...
		## variablendeklaration für prlcode / wird anfangs eingefügt
		...
		$newcode .= "my \$EVTPART1 = \"".ReadingsVal( $name, "EVTPART1", "" )."\";\n";
		$newcode .= "my \$EVTPART2 = \"".ReadingsVal( $name, "EVTPART2", "" )."\";\n";
		$newcode .= "my \$EVTPART3 = \"".ReadingsVal( $name, "EVTPART3", "" )."\";\n";
		$newcode .= "my \$EVENTFULL = \"".ReadingsVal( $name, "EVENTFULL", "" )."\";\n";
		$newcode .= "# deklaration ende\n";
		$newcode .=$2;
			
		$cs="{\n$newcode}";
			
		MSwitch_LOG($name,5,"Comand als Perlcode ausgeführt -> ". $cs . " ". __LINE__);
...
sub MSwitch_check_setmagic_i($$) {
    ...
    MSwitch_LOG( $name, 0, "vor setmagic: " . $msg );
    my $x = 0;
    while ( $msg =~ m/(.*)\[(.*)\:(.*)\:i\](.*)/ ) {
        $x++;    # notausstieg notausstieg
        last if $x > 20;    # notausstieg notausstieg
    ...
    MSwitch_LOG( $name, 0, "nach setmagic: " . $msg );
...
sub MSwitch_makegroupcmd($$) {
		my ( $hash, $gruppe ) = @_;
		my $Name = $hash->{NAME};
		#Log3( $Name, 0, "gruppe -  ". $gruppe);
		my @inhalt = split(/,/,$data{MSwitch}{$Name}{groups}{$gruppe});
		#Log3( $Name, 0, "gruppeninhalt - @inhalt - ". @inhalt);
		...
		#Log3( $Name, 0, "tmpdevices - @tmpdevices - ". @tmpdevices);
		push(@alldevices, @tmpdevices);

		}
		#Log3( $Name, 0, "alldevices - @alldevices - ". @alldevices); # enthält alle angesprochenen geräte
